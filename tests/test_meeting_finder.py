...
class TestWeeksAheadDateCalculation:
    ...
    def test_trip_duration_is_nights_plus_one(self, client):
        # Return date should be nights+1 days after outbound so that
        # e.g. 3 hotel nights → depart Mon, return Fri (4 days later).
        from datetime import date
        orig = app_module.SERPAPI_KEY
        app_module.SERPAPI_KEY = "test-key"
        for nights, expected_gap in [(3, 4), (1, 2), (0, 1)]:
            captured = {}
            def fake_serpapi(origin, dest, outbound_date, return_date):
                captured["outbound"] = outbound_date
                captured["return"]   = return_date
            ...
            assert gap.days == expected_gap, (
                f"nights={nights}: expected {expected_gap}-day gap, got {gap.days}"
            )
